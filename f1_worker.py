"""
F1 standings worker - fetches driver and constructor standings from the
Jolpica F1 API (https://api.jolpi.ca), an Ergast-compatible JSON service.

No scraping or HTML parsing needed; the API returns clean JSON.
"""
import logging
...
def fetch_drivers(season=None, cache_ttl=7200):
    ...
    for entry in raw_drivers:
        try:
            driver = entry["Driver"]
            # Constructors is a list; take the first (current team)
